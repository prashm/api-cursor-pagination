...
#       rows = paginated_rows(query_scope, 'row_id')
#
#       api_response = { status: 'Success', field_names: field_names }.
#         merge(paginated_meta(request.original_url.split('?').first, request.query_parameters)).
...
module ApiCursorPagination
  module Concern
    ...
    def validate_and_set_pagination_options(page_params)
      @errors ||= []
      self.page_size = 0
    ...
    def paginated_rows(scope, scope_id_str, row_id_str = scope_id_str)
      if page_size > 0
    ...
    def paginated_meta(base_url, query_params)
      if page_size > 0
        cursor = {}
