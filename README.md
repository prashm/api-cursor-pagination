...
[![Build Status](https://github.com/yourusername/api_cursor_pagination/workflows/CI/badge.svg)](https://github.com/yourusername/api_cursor_pagination/actions)
...
class UsersController < ApplicationController
  ...
  def index
    ...
    validate_and_set_pagination_options(params[:page])

    if @errors.blank?
      # Build your query scope
      scope = User.active.includes(:profile)
      
      # Get paginated results
      users = paginated_rows(scope, 'users.id')

      # Build API response with pagination metadata
      response = {
        status: 'Success',
        data: users.map(&:as_json)
      }.merge(paginated_meta(request.base_url + request.path, request.query_parameters))
...
users = paginated_rows(scope, 'cursor_id', 'id')
```
...
def index
  ...
  if @errors.blank?
    ...
    users = paginated_rows(scope, 'last_order_date')
...
#### `validate_and_set_pagination_options(params)`
...
#### `paginated_rows(scope, scope_id_str, row_id_str = scope_id_str)`
...
#### `paginated_meta(base_url, query_params)`
...
Bug reports and pull requests are welcome on GitHub at https://github.com/yourusername/api_cursor_pagination. This project is intended to be a safe, welcoming space for collaboration, and contributors are expected to adhere to the [code of conduct](https://github.com/yourusername/api_cursor_pagination/blob/main/CODE_OF_CONDUCT.md).
...
Everyone interacting in the ApiCursorPagination project's codebases, issue trackers, chat rooms and mailing lists is expected to follow the [code of conduct](https://github.com/yourusername/api_cursor_pagination/blob/main/CODE_OF_CONDUCT.md).
...
1. Check the [documentation](https://github.com/yourusername/api_cursor_pagination)
2. Search [existing issues](https://github.com/yourusername/api_cursor_pagination/issues)
3. Create a [new issue](https://github.com/yourusername/api_cursor_pagination/issues/new) if needed
